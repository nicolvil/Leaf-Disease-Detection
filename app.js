...
// ── INIT — model URL is hardcoded, no modal needed ─────────
...
// ── MODAL (kept for fallback, hidden by default) ───────────
function hideModal() {
...
async function loadModelFromInput() {
  ...
  if (!url) { showModalError('Please paste a Teachable Machine model URL.'); return; }
  setModelStatus('loading', 'Loading model…');
...
async function analyzeImage() {
  ...
  if (!ModelLoader.isReady()) return alert('Model is not ready yet. Please wait.');
...
function displayResults(predictions) {
  ...
  sorted.forEach(pred => {
    ...
    row.innerHTML = `
      ...
      <div class="prob-bar-track">
        <div class="prob-bar-fill ${pInfo.barClass}" style="width:0%" data-target="${pct}"></div>
      </div>
    `;
  ...
  // ← Tell the chatbot what disease was detected
  if (typeof notifyDiagnosis === 'function') {
    notifyDiagnosis(top.className);
  }
}
